...
exact registered task, resource revision, and loan/action binding from
`resource show`; see the [operator procedure](../../../../docs/resource-loans.md#resolve-an-unproven-ended-trainer).
Keep the document and operation ID unchanged after an unknown outcome. The
command refuses a running trainer.
