...
| `agent`, `model` | What ran. `model` may be `null`. |
| `thread` | The thread the event was addressed to. |
| `cwd` | The worker's working directory. |
| `evidence` | Absolute task directory. Contains `prompt.txt`, `output.log`, `exit.json`, `callback.log`. |
| `reports` | Worker reports in `seq` order, each `{"seq", "outcome", "summary"}`. `[]` when the worker never reported. |
| `process` | Tagged exit payload, or `null` for an interim report. |
| `next_action` | Suggested step. Follow it unless the reports say otherwise. |

`process` values: `{"kind":"exit","code":n}`, `{"kind":"signal","signal":n}`, `{"kind":"timeout","secs":n}`, `{"kind":"cancelled"}`, `{"kind":"spawn_failed","message":"…"}`, `{"kind":"runner_lost"}`.
...
| `TASK_SUCCEEDED` | `review_output` | Exit 0 and the last report was `succeeded`, or there were no reports. | Read the last summary. With `reports: []` the worker did not close the loop: read `output.log` before trusting the result. Verify the work in `cwd` (diff, tests) before telling the user it is done. |
...
| `TASK_FAILED` | `inspect_log` | Last report `failed`, non-zero exit, signal, timeout, or spawn failure. | Run `homebased task log <id> --tail 200`. Decide: fix the prompt and resubmit, raise `timeout`, fix the environment for `spawn_failed`, or report to the user. |
...
Delivery is at-least-once. A second message with the same `task` and `event` is a redelivery. Acknowledge it and take no new action. A `TASK_REPORTED` followed by an exit event for the same task is the normal two-message case, not a duplicate; the exit event carries the full `reports` list.

## Reading the evidence

```bash
homebased --json task show <id>          # status, exit_reason, reports, output_log, last_event
